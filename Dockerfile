...
RUN echo "=== Содержимое /opt/ts3server ===" && \
    ls -la /opt/ts3server && \
    echo "=== Проверка наличия entrypoint.sh ===" && \
    if [ -f /opt/ts3server/entrypoint.sh ]; then echo "entrypoint.sh найден"; else echo "entrypoint.sh НЕ найден"; fi
